...
class Custom_Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return transformed_data, 0 # We don't use label information.
    ...
    def select_dataloader(self):
        if 'celeb' in self.dataset:
            ...
            if 'imb' in self.dataset:
                ...
                # imb 1
                trainloader, testloader, tensorboard_imgs = self.load_imb_cifar10_dataset(self.path,imb_factor=1)
            ...
            train_mnist = datasets.MNIST(self.path, train=True, transform=mnist_transform, download=True)

            trainloader = torch.utils.data.DataLoader(dataset=train_mnist,
                                            batch_size=self.batch_size,
                                            shuffle=True)
            test_mnist = datasets.MNIST(self.path, train=False, transform=mnist_transform, download=True)
