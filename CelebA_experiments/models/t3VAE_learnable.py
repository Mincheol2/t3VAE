...
class t3VAE_learnable(baseline.VAE_Baseline):
    def __init__(self, image_shape, DEVICE, args):
        super(t3VAE_learnable, self).__init__(image_shape, DEVICE, args)
        ...
        log_tau_base = -self.n_dim * np.log(self.args.prior_sigma) + self.log_t_normalizing_const(self.args.nu,self.n_dim)
        log_tau_base += - np.log(self.args.nu + self.n_dim - 2) + np.log(self.args.nu-2)
        
        self.const_2bar1_term_1 = (1 + self.m_dim / (self.args.nu + self.n_dim -2))
        const_2bar1_term_2_log = -self.gamma / (1+self.gamma) * log_tau_base

        self.const_2bar1 = self.const_2bar1_term_1 * const_2bar1_term_2_log.exp()
        ...
        self.logsigma = torch.nn.Linear(self.n_dim , 1)
    ...
    def decoder(self, z):
        ...
        self.args.prior_sigma = self.logsigma(z.flatten(start_dim=1)).exp()
        return z
    ...
    def reparameterize(self, mu, logvar):
        ...
        std = torch.tensor(self.args.nu / self.nu_prime).sqrt() * std
    ...
    def loss(self, x, recon_x, z, mu, logvar):
        ...
        # For learnable sigma
        log_tau_base = -self.n_dim * torch.log(self.args.prior_sigma) + self.log_t_normalizing_const(self.args.nu,self.n_dim)
        log_tau_base += - np.log(self.args.nu + self.n_dim - 2) + np.log(self.args.nu-2)
        
        const_2bar1_term_2_log = -self.gamma / (1+self.gamma) * log_tau_base

        self.const_2bar1 = (self.const_2bar1_term_1 * const_2bar1_term_2_log.exp()).squeeze()
        log_tau = 2 / (self.args.nu + self.n_dim - 2) * log_tau_base
        self.tau = log_tau.exp().squeeze(1)
        ...
        # Now we should consider sigma in C_1
        C_1_sigma = self.args.prior_sigma.pow(-self.n_dim * self.gamma / (1 + self.gamma))

        reg_loss = mu_norm_sq + trace_var - self.args.nu * self.const_2bar1 * log_det_var.exp() + self.args.nu * self.tau

        reg_loss = torch.mean(C_1_sigma * reg_loss)
        # reg_loss = self.args.beta_weight * reg_loss
        ## recon loss (same as VAE) ##

        recon_loss = (recon_x - x)**2 / (N * self.args.prior_sigma.reshape(-1,1,1,1).pow(2))
        recon_loss = torch.sum(C_1_sigma.reshape(-1,1,1,1) * recon_loss)
