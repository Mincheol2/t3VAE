...
parser.add_argument('--xmin',           type=float, default=-15.0,   help="Minimum value of x-axis")
parser.add_argument('--xmax',           type=float, default=20.0,   help="Maximum value of y-axis")
parser.add_argument('--ymin',           type=float, default=-15.0,   help="Minimum value of x-axis")
...
sample_var_list = [
    torch.tensor([[4.0,3.0],[3.0,4.0]]), 
    torch.tensor([[5.0,1.0],[1.0,1.0]])
]

model_list = [
    t3VAE.t3VAE(n_dim = n_dim, m_dim = 2, nu=25.0, recon_sigma=0.65, device=device).to(device), 
    t3VAE.t3VAE(n_dim = n_dim, m_dim = 2, nu=25.0, recon_sigma=0.75, device=device).to(device), 
    t3VAE.t3VAE(n_dim = n_dim, m_dim = 2, nu=25.0, recon_sigma=0.85, device=device).to(device), 
    TVAE.TVAE(n_dim = n_dim, m_dim = 2, device=device).to(device) 
    # Disentangled_VAE.Disentangled_VAE(n_dim = n_dim, m_dim = 2, nu = 10.0, recon_sigma=0.75, device=device, sample_size_for_integral=1).to(device), 
    # VAE.VAE(n_dim = n_dim, m_dim = 2, recon_sigma=0.5, device=device).to(device)
]
