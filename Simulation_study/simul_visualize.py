...
def visualize_density_simple(train_data, test_data, model_nu_list, 
    ...
    plt.hist(test_data, bins = 100, range = [-20, 20], alpha = 0.5, density=True, label = "Test")
    plt.xlim(-20, 20)
    plt.title(f'Train/test data (nu = {sample_nu_list})')
    ...
    plt.xlim(-xlim, xlim)
    plt.yscale("log")
    plt.title('Train/test data (log scale)')
...
def visualize_latent(sample_nu, test_latent, test_data, model_nu_list, gAE_latent_list, VAE_latent, gAE_recon_list, VAE_recon, xlim) :
    ...
    for m in range(M) : 
        ax = fig.add_subplot(2,M+2,m+2)
        plt.scatter(gAE_latent_list[m][:,0], gAE_latent_list[m][:,1])
        ax.axis(domain1)
        plt.title(f'gAE latent (nu = {model_nu_list[m]})')

        ax = fig.add_subplot(2,M+2,M+m+4, projection='3d')
        plt.scatter(gAE_recon_list[m][:,0], gAE_recon_list[m][:,1], gAE_recon_list[m][:,2])
        ax.axis(domain2)
        ax.set_zlim(zlim)
        plt.title(f'gAE reconstruction')

    ax = fig.add_subplot(2,M+2,M+2)
    plt.scatter(VAE_latent[:,0], VAE_latent[:,1])
    ax.axis(domain1)
    plt.title(f'VAE latent (nu = {model_nu_list[m]})')

    ax = fig.add_subplot(2,M+2,2*M+4, projection='3d')
    plt.scatter(VAE_recon[:,0], VAE_recon[:,1], VAE_recon[:,2])
    ax.axis(domain2)
    ax.set_zlim(zlim)
